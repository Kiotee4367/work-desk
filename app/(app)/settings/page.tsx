...
export default async function SettingsPage({ searchParams }: PageProps<"/settings">) {
  ...
  const error = params.error === "brand";
    ...
    <div className="flex flex-col gap-8">
      ...
      <section aria-labelledby="brand-heading" className="rounded-xl border border-ink/10 p-4">
        ...
        <p className="mt-1 text-sm text-ink/80">
          Choose whose colors, logo and name the app shows. Everything updates as soon as you save.
        </p>

        {saved && (
          <p role="status" className="mt-3 rounded-lg border border-accent bg-accent/10 px-3 py-2 text-sm">
            Brand saved. You are now on {active.name}. Next step: fill in your Brand library.
          </p>
        )}
        {error && (
          <p role="alert" className="mt-3 rounded-lg border border-red-600 bg-red-50 px-3 py-2 text-sm">
            That brand is not on the list. Pick one below and save again.
          </p>
        )}
              ...
              {BRAND_LIST.map((b) => (
                ...
                >
                  <input
                    ...
                    name="brand"
                    value={b.key}
                    defaultChecked={b.key === active.key}
                    className="mt-1"
                  />
        ...
        <p className="mt-3 text-xs text-ink/60">
          Adding a client brand is one edit to config/brands.ts. No other file changes.
        </p>
      </section>

      <section aria-labelledby="status-heading" className="rounded-xl border border-ink/10 p-4">
        <h2 id="status-heading" className="text-lg font-semibold">Setup status</h2>
          ...
          {statuses.map((s) => (
            <li key={s.name} className="flex flex-col gap-1 rounded-lg border border-ink/10 px-3 py-2 text-sm sm:flex-row sm:items-start sm:gap-3">
              <span className="flex items-center gap-2 sm:w-52 sm:shrink-0">
                <span
                  aria-hidden="true"
                  className={`inline-block h-2.5 w-2.5 rounded-full ${s.ready ? "bg-green-600" : "bg-amber-500"}`}
                />
